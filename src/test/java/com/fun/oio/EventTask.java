...
public class EventTask implements Runnable {
      ...
      for (; ; ) {
        try {
          ...
          if (line != null) {
            // 通过输入流接收客户端信息
            System.out.println(
                "[CommandCenter] receive a socket task:" + line + "," + socket.getInetAddress());
          }
          ...
          RecordLog
              .info("[CommandCenter] deal a socket task:" + line + "," + socket.getInetAddress());
        } catch (Exception ex) {
  ...
  private Request parseRequest(String line) {
    ...
    String parameterStr = line
        .substring(ask != -1 ? ask + 1 : 0, space != -1 ? space : line.length());
