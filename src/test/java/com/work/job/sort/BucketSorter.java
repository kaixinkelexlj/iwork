...
public class BucketSorter {
	...
	public void sort(int[] keys, int from, int len, int max) {
		...
		for (int i = 1; i < max; i++) {
			count[i] = count[i] + count[i - 1];// this means how many number
												// which is less or equals than
												// i,thus it is also position +
												// 1
		}
		System.arraycopy(keys, from, temp, 0, len);
		for (int k = len - 1; k >= 0; k--)// from the ending to beginning can
											// keep the stability
		{
